...
void save_done(const struct Item item[], char fileName[], int NoOfRecs);

// Function Prototypes Milestone 5:

void deleteItem(struct Item item[], int NoOfRecs);
void searchInventory(struct Item item[], int* NoOfRecs);
...
void GroceryInventorySystem(void) {
	...
	struct Item list[MAX_ITEM_NO];
	int NoOfRecord_read;
	int status;
	welcome();
	status = loadItems(list, DATAFILE, &NoOfRecord_read);
	...
	if (status == 1) {
		...
		do {
			options = menu(); //calls menu() function and store user input for switch
			switch (options) {
			case 1:
				
				listItems(list, NoOfRecord_read);
				pause();
				break;

			case 2:
				
				search(list, NoOfRecord_read);
				pause();
				break;

			case 3:
				
				adjustQuantity(list, NoOfRecord_read, CHECKOUT);
				save_done(list, DATAFILE, NoOfRecord_read);
				pause();
				break;
			...
			case 4:
				...
				adjustQuantity(list, NoOfRecord_read, STOCK);
				save_done(list, DATAFILE, NoOfRecord_read);
				pause();
				break;
			...
			case 5:
				...
				addOrUpdateItem(list, &NoOfRecord_read);
				save_done(list, DATAFILE, NoOfRecord_read);
				pause();
				break;
			...
			case 6:
				...
				deleteItem(list, NoOfRecord_read);
				status = loadItems(list, DATAFILE, &NoOfRecord_read);
				pause();
				break;
			...
			case 7:
				...
				searchInventory(list, &NoOfRecord_read);
				break;
...
struct Item itemEntry(int sku) {
	...
	printf("       Name: ");
	scanf(" %[^\n]s", I.name);   // for some reason matrix generated an error without s
	flushKeyboard();

	printf("      Price: ");
	I.price = getDoubleLimited(0.01, 1000.00);
...
// didisplay all items in array with warning for thos with low stock/quantity level

void displayItem(struct Item item, int linear) {

	switch (linear) {			// I was previously using if else
								// using switch rather than if else because this way the code is more efficient
		case 1:
...
// This function reads six fields form the text file if successful return 1 otherwise 0

int loadItem(struct Item* item, FILE* dataFile) {

	int rec_read=0; // stores the toatl number of record read or in our case the total number of fields read
	int done;

	rec_read = fscanf(dataFile, "%d,%d,%d,%lf,%d,%[^\n]%*c", &item->sku, &item->quantity, &item->minQuantity, &item->price, &item->isTaxed, item->name);
			// fscanf reads fields in text file and store number of fields read in rec_read
	if (rec_read == 6) {

		done = 1;		// assign the return variable value 1 if 6 fields are read
...
// This function calls the saveItem function for wirting array of items into the text file but overwrite the old record

int saveItems(const struct Item item[], char fileName[], int NoOfRecs) {

	int i;
	int no_error = 0; // if the file fails to open return 0 ( as an error) ,  0 here means false so an error exist
	FILE *fp = NULL;
	...
	if (fp != NULL) {
		...
		for (i = 0; i < NoOfRecs; i++) {
			...
			if (item[i].sku != 0) {
				...
				saveItem(item[i], fp); // calls saveItem function and overwrite the old data with new data/records
...
//This function calls the loadItem function for reading the record in the text file

int loadItems(struct Item item[], char fileName[], int* NoOfRecsPtr) {

	int no_error = 0; // if the file fails to open return 0 ( as an error).  0 here means false so an error exist
	int rec_read = 0; // stors the total umber of recods or fields read
	FILE *fp = NULL;
	...
	if (fp != NULL) {
		...
		while (loadItem(&item[rec_read], fp) == 1) {

			rec_read++;

		}

		*NoOfRecsPtr = rec_read; // pass the number of records read to the pointer
		no_error = 1; // 1 for ture. so, no error exist
		fclose(fp); //closes the file
	}
	else {
...
int main(void) {
	...
	GroceryInventorySystem(); // calls GroceryInventory
	return 0;
}

// the function calls saveItems function for saving the data

void save_done(const struct Item item[], char fileName[], int NoOfRecs) {

	int no_error;
	no_error = saveItems(item, DATAFILE, NoOfRecs); // calls saveItems function
	if (no_error == 0) {
...
// Function implementation Milestone 5

// This function search the record with the provided Sku from the user and delete it if uer enter "y" or "Y"
...
void deleteItem(struct Item item[], int NoOfRecs) {
	...
	if (found == 1) {
		...
		if (sure == 1) {
			...
			item[Index].sku = 0; // sets the specific sku to null so that the saveItem funtion will ignore it when saving to file
		    
			done = saveItems(item, DATAFILE, NoOfRecs);	//Calls saveItems function to store the new data
			status = loadItems(item, DATAFILE, &NoOfRecs);	//Calls loadItems function to load the items to array again replacing the old ones
...
//Function takes user argument and search through and display the items that matches that name or are part of some same
...
void searchInventory(struct Item item[], int* NoOfRecs) {
	...
	scanf("%[^\n]s", name);
	flushKeyboard(); //Clears the memory buffer
	for (i = 0; i < *NoOfRecs; i++) {

		if (strstr(item[i].name, name) != NULL) {  // check the avability of second string in firts string (new commadn learnd told in Milestone 5)
			
			found = 1;
			displayItem(item[i], 0);  // sends each found record index to display function for displaying
			pause();
